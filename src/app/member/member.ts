...
export class Member {
  ...
  selectedMember = signal<MemberRecord | null>(null);
  ...
  statusLabel: Record<MemberStatus, string> = {
    active:   'Activo',
    expiring: 'Por vencer',
    expired:  'Vencido',
  };

  statusClass: Record<MemberStatus, string> = {
  ...
  confirmDelete(): void {
    ...
    if (!member) return;
    this.members.update(list => list.filter(c => c.id !== member.id));
    this.closeModal();
  }
